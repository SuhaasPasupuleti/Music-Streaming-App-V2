...
cache=Cache(app,config={'CACHE_TYPE':'redis','CACHE_REDIS_URL':'redis://localhost:6379/1','CACHE_DEFAULT_TIMEOUT':600})
...
with app.app_context():
    ...
    if not datastore.find_user(username="admin"):
        datastore.create_user(username='admin',email='21f1001693@ds.study.iitm.ac.in',password=generate_password_hash('edoti'),name='Administrator',roles=["admin"])
        db.session.commit()
...
def send_reminders():
    with app.app_context():
        ...
        users.pop(0)
        for user in users:
            send_email('suhaas.pasupuleti@gmail.com',user.email,'Daily Reminder','Hey! its been a while since u listened to music,Get back to listening to good music on our platform.')
...
def send_email(sender,receiver,subject,message,attachment=None):
    ...
    smtp_password="wcycageneanqfmpm"
...
def send_report(sender,**kwargs):
    sender.add_periodic_task(30,send_email.s('suhaas.pasupuleti@gmail.com','21f1001693@ds.study.iitm.ac.in','report','The report of the music streaming app is attached below.','report.csv'))
    sender.add_periodic_task(30,create_report.s())
    sender.add_periodic_task(30,send_reminders.s())
...
def admindash():
    ...
    for i in users:
        ...
        if 'user' in i.roles:
            user.append(i)
    songcount=Song.query.all()
    songs=Song.query.order_by(desc(Song.rating)).all()
    return [marshal(creator,user_fields),marshal(user,user_fields),marshal(songcount,songs_fields),marshal(songs,songs_fields)]
