export default{
    ...
    <div>
        ...
        <div class="container mt-3" v-if="albumsongs.length>0">
                ...
                <div v-for="song in albumsongs"  class="outer" style="margin: 2rem;padding-top: 18px;width: 10rem;">
                    <div class="col-2 box p-2 song" style="box-shadow: 7px 5px 50px rgba(0,0,0,0.2)!important;border-radius: 20px;height: 7rem;width: 10rem;">
                        ...
                        <div class="butn row d-flex pt-1 p-2 text-center">
                            ...
                            <a @click="removefromalbum(song.id)"  class='btn col-6 btn-danger'>
                                Remove
                            </a>
                        </div>
                    </div>
        ...
        <div class="container mt-2" v-if="newsongs.length>0">
            <h1>Add to album</h1>
            <div  style="display: flex;flex-direction:row;overflow: scroll;overflow: hidden;white-space: nowrap;height: 15rem;">
                <div v-for="song in newsongs"  class="outer" style="margin: 2rem;padding-top: 18px;width: 10rem;">
                    <div class="col-2 box p-2 song" style="box-shadow: 7px 5px 50px rgba(0,0,0,0.2)!important;border-radius: 20px;height: 7rem;width: 10rem;">
                        ...
                        <div class="butn row d-flex pt-1 p-2 text-center">
                            ...
                            <a @click="addtoalbum(song.id)"  class='btn col-6 btn-success'>
                                ADD
                            </a>
                        </div>
                    </div>
    ...
    data(){
        ...
        const url = '/albums'+`/${this.albumid}`
        const albumres = await fetch(url)
        albumres.json().then((data)=>{this.album=data[0],this.newsongs=data[1],this.albumsongs=data[2]}).catch((error)=>{console.log(error)})
    },
    methods:{sendid (songid){
        ...
        async addtoalbum(songid){
            this.songid=songid
            const url = '/albums'+`/${this.albumid}/${this.songid}`
            const response = await fetch(url,{
                method:'POST'
            })
            if(response.ok){console.log("added to album");location.reload()}
        },
        async removefromalbum(songid){
            ...
            if(response.ok){console.log("removed from album");location.reload()  }
        }
